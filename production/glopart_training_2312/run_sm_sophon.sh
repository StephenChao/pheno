...
if [[ $MACHINE == "farm" ]]; then
    ...
    TUPLE_DIR=/publicfs/cms/user/$USER/ntuplizer/sophon/analyzers
...
generate_tuple(){
    ...
    root -b -q 'analyze.C++("events_delphes.root", "tuple.root", "JetClassII_Sophon.onnx")'
}
...
for ((i=0; i<nbatch; i++)); do
    ...
    # run ntuplilzer
    if [ $? -eq 0 ]; then
        # if successful, run ntuplilzer
        cp -r $TUPLE_DIR/* ./
        generate_tuple
        mv events_delphes.root $WORKDIR/events_delphes_$i.root
        mv tuple.root $WORKDIR/tuple_$i.root
    fi
    ...
    if [ $(((i+1) % 100)) -eq 0 ]; then
        ...
        hadd -f $WORKDIR/merged_tuple_$i.root $WORKDIR/tuple_*.root
        if [ $? -eq 0 ]; then
            rm -f $WORKDIR/events_delphes_*.root
            rm -f $WORKDIR/tuple*.root            
        fi
    fi
...
if [ $nbatch -eq 1 ]; then
    ...
    mv $WORKDIR/tuple_0.root tuple.root
else
    hadd -f events_delphes.root $WORKDIR/events_*.root
    hadd -f tuple.root $WORKDIR/tuple*.root
fi

mkdir -p $OUTPUT_PATH/$PROC
mkdir -p $OUTPUT_PATH_TUPLE/$PROC


# transfer the file
mv -f events_delphes.root $OUTPUT_PATH/$PROC/events_delphes_$JOBNUM.root
mv -f tuple.root $OUTPUT_PATH_TUPLE/$PROC/tuple_$JOBNUM.root
