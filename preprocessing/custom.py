...
class CustomDelphesSchema(BaseSchema):
    ...
    def _build_collections(self, branch_forms):
        ...
        # print(f"branch_forms:{branch_forms}")
        # parse into high-level records (collections, list collections, and singletons)
        collections = set(k.split("/")[0] for k in branch_forms)
        collections -= set(k for k in collections if k.endswith("_size"))
        
        #branchs start with "jet_" are not collections
        jet_branchs = set(k for k in collections if k.startswith("jet_"))
        # print(f"jet_branchs:{jet_branchs}")
        collections -= jet_branchs
